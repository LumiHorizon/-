...
把 B站博主**青枫浦上Q（赛博青哥）**的早盘/复盘/视频内容，沉淀成一套**可被 AI 直接调用的 A股投研系统**：自动抓取博主观点 → 蒸馏成结构化框架 → 配合实时行情，让 Claude Code 按博主的方法论帮你分析个股。

## 这个项目能做什么

- **自动追更博主观点**：脚本定时抓取博主的早盘/复盘动态（含付费长文）和视频字幕，沉淀为原始语料，`--resume` 增量累积、不覆盖历史。
- **蒸馏成可复用框架**：把零散内容提炼成结构化的方法论与规律库（情绪周期、量能台阶、选股纪律、产业趋势……），按场景分类、带快速索引。
- **按框架分析个股**：接入东方财富实时行情/K线，让 Claude 判断"某标的是否符合博主当前进攻方向"、解读盘面强弱、给出仓位纪律。
- **选股扫描**：七轨布林线扫描，批量筛出当前处于买入区的标的。
- **一键出图**：把复盘要点生成 PPT（python-pptx 模板）。
- **跨项目/跨机复用**：内置 Claude Code Skill，任意项目 clone 后即可让 AI 自动读框架+最近蒸馏+取数后作答。
